...
| **`symlink`**<br>Clone + symlink | Clone the repo yourself, then symlink individual skills from `~/.agents/skills` (and `~/.claude/skills`, `~/.gemini/config/skills`, or `~/.config/opencode/skills`) into it. | Manual, but full control — and it's what lets you layer `skills-sdlc`'s override of `/implement` on top of the same-named upstream skill. |
...
### Script Helpers
- `./scripts/install-skills.sh` — audits every skill reference in your docs, links matching skills into `~/.agents/skills` (and `~/.claude/skills`, `~/.gemini/config/skills`, `~/.config/opencode/skills`)
- `./scripts/install-agents.sh` — links the `planner` / `implementer` agent definitions for Claude, Codex, Pi, Antigravity, and OpenCode
